...
func init() {
	pflag.BoolVar(&Delete, "delete", false, "delete a key given its id")
	pflag.BoolVar(&List, "list", false, "list all keys for user")
	pflag.StringVar(&ID, "id", "", "username or key id")
}
...
func main() {
	...
	// Check username or key id is set.
	if ID == "" {
		os.Exit(1)
	}

	if List {
		if err := conf.GetUserPubKeys(ID); err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
	} else if Delete {
		if err = conf.DeletePubKey(ID); err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
	}
