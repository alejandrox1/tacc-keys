...
#-                       :-) A Container Cluster (-:
#-
#- Emulate a cluster via containers.
#- 
##
## Usage:
##
...
##      
##      . ./config.sh
##      ssh -o "StrictHostKeyChecking no" -i ssh/id_rsa -p $CLUSTER_SSH_PORT docker@localhost
##
set -e
set -o pipefail
...
exec_on_mpi_master() {
    docker exec -it -u docker $(docker-compose ps | grep 'master'| awk '{print $1}') "$@"
}
...
if [ $cmd_up == 1 ]; then
    ...
    exec_on_mpi_master bash

elif [ $cmd_exec == 1 ]; then
    echo -e "${blue}${ASCII_ART_EXEC}${reset}"
    exec_on_mpi_master bash -c "${SHELL_COMMAND}"
